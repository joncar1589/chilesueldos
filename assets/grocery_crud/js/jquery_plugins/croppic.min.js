...
(function (window, document) {
	...
	Croppic = function (id, options,input) {
	...
	Croppic.prototype = {
		...
		init: function () {
			...
			that.bindImgUploadControl();
                ...
                edit: function(){
                    var that = this;	
                    that.res = 0;                    
                    that.input = $("#"+that.id+" input");
                    that.src = url+"/"+crop_upload_url+"/"+that.input.val()
                    that.obj.prepend('<img class="croppedImg" src="'+url+"/"+crop_upload_url+"/"+that.input.val()+'">');
                    if(that.options.outputUrlId !== ''){$('#'+that.options.outputUrlId).val(crop_upload_url+"/"+that.input.val());}
		...
		bindImgUploadControl: function(){
			...
			that.form = $('#crudForm');
			if(that.res>0){                            
                            that.obj.append('<input id="'+that.input.attr('id')+'" type="file" name="'+that.input.attr('name')+'" style="visibility:hidden">');
                            that.input = $("#"+that.id+" input");
                        }
			...
			if( !$.isEmptyObject(that.croppedImg)){
				...
				that.cropControlRemoveCroppedImage.on('click',function(){ 
					...
					if(that.options.outputUrlId !== ''){	$('#'+that.options.outputUrlId).val('');	}
                                        $.post(that.options.deleteUrl,{url:that.src,path:crop_upload_url,field:that.input.attr('name')},function(data){});
                                        that.src = undefined;
                                        that.input.attr('type','file');
                                        that.input = $("#"+that.id+" input");
                                        that.croppedImg.remove();
				});	
			...
			that.input.change(function(){
				...
				//var formData = new FormData(that.form[0]);
                                ...
                                formData.append('url',crop_upload_url);
                                formData.append('img',that.input.parent('div').find('input[type=file]')[0].files[0]);
		...
		createCropControls: function(){
			...
			that.cropControlReset.on('click',function(){ that.reset(); $.post(that.options.deleteUrl,{url:that.src,path:crop_upload_url,field:that.input.attr('name')},function(data){});});				
                ...
                type: 'POST'
					...
					if(response.status=='success'){
						...
						that.hideLoader();
                                                ...
                                                that.input.attr('type','input');
                                                url = response.url;
                                                url = url.split('/');
                                                url = url[url.length-1];
                                                that.input.attr('value',url);
                                                that.input = that.input = $("#"+that.id+" input");
