<?php
$this->set_css($this->default_theme_path . '/liquidaciones/css/flexigrid.css');
...
$this->set_js($this->default_theme_path . '/liquidaciones/js/cookies.js');
$this->set_js($this->default_theme_path . '/liquidaciones/js/flexigrid.js');
$this->set_js($this->default_theme_path . '/liquidaciones/js/jquery.form.js');
$this->set_js($this->default_javascript_path . '/jquery_plugins/jquery.numeric.min.js');
$this->set_js($this->default_theme_path . '/liquidaciones/js/jquery.printElement.min.js');
    ...
    <div id='main-table-box' class="main-table-box">
        ...
        <?php if (!$unset_add || !$unset_export || !$unset_print) { ?>
            <div class="tDiv">
                ...
                <?php } ?>
                <!-- Boton para reenviar la notificacion cuando el cliente haya terminado de acer las modificaciones -->
                <div class="tDiv3">
                    <div class="btnseparator"></div>
                     <script language="javascript">
                        function reenviar() {
                            var confirmar = confirm("¿Decea Validar la informacion contenida en las liquidaciones de sus trabajadores?");
                            if(confirmar == true){ 
                                //var empresa = 
                                $.post("http://www.chilesueldos.cl/panel/validar_liquidacion/<?php echo str_replace('/','-',$_SERVER['REQUEST_URI']) ?>", {}, function (data){emergente(data)});
                                $.post("http://www.chilesueldos.cl/main/notificar/9/1/<?php echo substr($_SERVER['REQUEST_URI'],strrpos($_SERVER['REQUEST_URI'],"/")+1);?>", {}, function () {});
                                alert("Su notificación ha sido actualizada, por favor revise su correo.")
                            }
                        }
                    </script>
                    <a class="" onclick="javascript:reenviar()">
                        <div class="fbutton">
                            <div>
                                <span class="reenviar" id="reenviar">Validar Liquidaciones</span>
                            </div>
                        </div>
                    </a>
                    <div class="btnseparator"></div>
                    <a class="" href="http://www.chilesueldos.cl/empresa/liquidaciones">
                        <div class="fbutton">
                            <div>
                                <span class="otrosMeses">Buscar Otros Meses</span>
                            </div>
                        </div>
                    </a>
                    <div class="btnseparator"></div>
