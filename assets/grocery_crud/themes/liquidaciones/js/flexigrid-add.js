...
$(function(){
		...
		$('#crudForm').submit(function(){
			var my_crud_form = $(this);
                        if(load==0)
                        {
                            alert("Aguarde un momento, el explorador no ha culminado su carga. Una vez cargado podrá guardar el registro")
                            return false;
                        }
                        else{
                        if(confirm('Esta seguro que desea añadir esta liquidacion?')){
			$(this).ajaxSubmit({
				url: validation_url,
				dataType: 'json',
				cache: 'false',
