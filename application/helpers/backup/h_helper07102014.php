            ...
            if($empresas->num_rows>0){
			  ...
			  /*-----------------------------------------------------------------------------------------------------------------------------------------------------*/
			   $fecha_emision = date("d")<=13?date("Y-m-d",strtotime(date("Y-m-t",strtotime("-1 month ".date("Y-m"))))):date("Y-m-d",strtotime(date("Y-m-t")));
                        $empleado = get_instance()->querys->get_salario($e->id);
                        $iduf = date("d")<=13?((date("Y")-2000)*date("Y"))+date("m",strtotime("-1 month ".date("Y-m"))):((date("Y")-2000)*date("Y"))+date("m");
			  ...
			  /*-----------------------------------------------------------------------------------------------------------------------------------------------------*/
                        if($empleado->num_rows>0 && $dbuf->num_rows>0 && $salario->num_rows>0 && $dbfamiliares->num_rows>0 && $dbimpuestos->num_rows>0 && !empty($dbsca) && !empty($dbsalud)){
                        ...
                        //Calcular % de salud
                        $salud = $total_imponible <= $tope_imponible?$total_imponible*($empleado->por_salud/100):$tope_imponible*($empleado->por_salud/100);
                        //Calcular apv	
                        ...
                        $legal_7 = $total_imponible<=$tope_imponible?$total_imponible*($empleado->por_salud/100):$tope_imponible*($empleado->por_salud/100);
                        ...
                        $total_leyes_sociales = $fondopensiones+$salud+$apv+$inp+$segurocesantia+$adicional_salud;        
                        ...
                        $ips = $cot_accidente_trabajo-$familiares;
                        //Cotizacion obligatoria            
                        //Adicional de salud        
                        $total_fonasa = $ips<0?$adicional_salud+$salud+$ips:$adicional_salud+$salud;
                        //Total leyes sociales
                        $total_leyes_sociales = $total_fonasa<0?$afp+$afc:$afp+$afc+$ips+$total_fonasa;
                        //Costo total empleador
                        $costo_total_empleador = $total_leyes_sociales+$otros_descuentos;                
