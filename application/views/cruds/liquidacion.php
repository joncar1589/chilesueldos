...
<script>
    ...
    $(document).ready(function () {
        if ($("#field-factor_hora_extra").val() == 0) {
            $("#field-factor_hora_extra").val(1.5);
        }
        $("#field-factor_hora_extra").change(function () {
            if (parseFloat($(this).val()) < 1.5)
            {
                alert("Este valor debe ser un numero mayor o igual que 1.5")
                $(this).focus();
            }
        });
...
<?php endif ?>
        ...
        if ($("input:radio[name=check_horas_extras]:checked").val() == '0') {
            $("#cantidad_horas_field_box, #factor_hora_extra_field_box, #hrs_extras_field_box").hide();
            $("#field-cantidad_horas, #field-hrs_extras").val(0);
        }

        $("input:radio[name=check_horas_extras]").change(function () {
            if ($(this).val() == 1) {
                $("#cantidad_horas_field_box, #factor_hora_extra_field_box, #hrs_extras_field_box").show("slow");
            }
            else {
                $("#cantidad_horas_field_box, #factor_hora_extra_field_box, #hrs_extras_field_box").hide("fast");
                $("#field-cantidad_horas, #field-hrs_extras").val(0);
            }
        });


        if ($("input:radio[name=check_bonos]:checked").val() == '0') {
            $("#comisiones_field_box, #bonos_field_box, #aguinaldos_field_box").hide();
            $("#field-comisiones, #field-bonos, #field-aguinaldos").val(0);
        }

        $("input:radio[name=check_bonos]").change(function () {
            if ($(this).val() == 1) {
                $("#comisiones_field_box, #bonos_field_box, #aguinaldos_field_box").show("slow");
            }
            else {
                $("#comisiones_field_box, #bonos_field_box, #aguinaldos_field_box").hide("fast");
                $("#field-comisiones, #field-bonos, #field-aguinaldos").val(0);
            }
        });
        
        if ($("input:radio[name=check_descuentos]:checked").val() == '0') {
            $("#cta_ahorro_afp_field_box, #anticipos_field_box, #prestamos_ccaf_field_box, #descuentos_prestamos_field_box, #otros_descuentos_field_box").hide();
            $("#field-cta_ahorro_afp, #field-anticipos, #field-prestamos_ccaf, #field-descuentos_prestamos, #field-otros_descuentos").val(0);
        }

        $("input:radio[name=check_descuentos]").change(function () {
            if ($(this).val() == 1) {
                $("#cta_ahorro_afp_field_box, #anticipos_field_box, #prestamos_ccaf_field_box, #descuentos_prestamos_field_box, #otros_descuentos_field_box").show("slow");
            }
            else {
                $("#cta_ahorro_afp_field_box, #anticipos_field_box, #prestamos_ccaf_field_box, #descuentos_prestamos_field_box, #otros_descuentos_field_box").hide("fast");
                $("#field-cta_ahorro_afp, #field-anticipos, #field-prestamos_ccaf, #field-descuentos_prestamos, #field-otros_descuentos").val(0);
            }
        });
        
        $("#field-alcance_liquido, #field-total_leyes_sociales, #field-costo_total_empleador, #field-hrs_extras").attr('readonly', true);
        $("#field-alcance_liquido, #field-total_leyes_sociales, #field-costo_total_empleador").css({"background": "#C3C3C3", 'color': 'white'});
...
//           var dias = new Date(parseInt(f[2]) || new Date().getFullYear(),parseInt(f[1]), 0).getDate();
    ...
    function calculos()
    {
        if (empleado != undefined && dbsca != undefined && dbuf != undefined && dbfamiliares != undefined && dbimpuestos != undefined && salario != undefined)
        {
            ...
            //------------------------------------------------------------
            var bonosImp = bonos + comisiones + aguinaldos;
            $("#field-total_bonos_imp").val(!isNaN(bonosImp) ? bonosImp.toFixed(0) : 0);
            //------------------------------------------------------------
            ...
            fondopensiones = empleado['check_afp'] !== '1' ? 0 : fondopensiones;
            ...
            segurocesantia = empleado['check_afc'] == '1' ? segurocesantia : 0;
            ...
            //Descuentos (sin impuesto de segunda categoria)
            var descuentos = parseFloat($("#field-cta_ahorro_afp").val()) + parseFloat($("#field-anticipos").val()) + parseFloat($("#field-prestamos_ccaf").val()) + parseFloat($("#field-descuentos_prestamos").val()) + parseFloat($("#field-otros_descuentos").val());
            $("#field-total_otr_desc_2").val(!isNaN(descuentos) ? descuentos.toFixed(0) : 0);
            ...
            seguro_sis = empleado['tipo_trabajador'] == 'Activo' ? seguro_sis : 0;
            ...
            seguro_cesantia_empleador = empleado['check_afc'] == '1' ? seguro_cesantia_empleador : 0;
