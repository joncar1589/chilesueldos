...
<script>
    ...
    $(document).ready(function(){
       ...
       $("#field-fecha").change(function(){consultas($(this).val());});
       //Ajustes de interfaz
       ...
       $("input").change(function(){calculos()});
       <?php if(empty($y)): ?>
...
//           var dias = new Date(parseInt(f[2]) || new Date().getFullYear(),parseInt(f[1]), 0).getDate();
           ...
           $.post('<?php echo base_url('empresa/getuf') ?>',{id:iduf},function(data){
               ...
               else alert("No se consiguieron registros de UF para el mes seleccionado");
           ...
           $.post('<?php echo base_url('empresa/get_salario_minimo') ?>',{mes:f[1],ano:f[2]},function(data){
               ...
               else alert("No se consiguieron Salarios Minimos para el mes seleccionado");                   
           }); 
           ...
           $.post('<?php echo base_url('empresa/getfamiliares') ?>',{ano:f[2]},function(data){
               dbfamiliares = JSON.parse(data);
                ...
                else alert("No se consiguieron registros de familiares para el mes seleccionado");
           });
           //Traer impuestos
           $.post('<?php echo base_url('empresa/getimpuestos') ?>',{mes:f[1],ano:f[2]},function(data){                
                ...
                else alert("No se consiguieron registros de impuestos para el mes seleccionado");
           });
           ...
           $.post('<?php echo base_url('empresa/getsca') ?>',{mes:f[1],ano:f[2],empresa:$("#field-empresa").val()},function(data){                
                ...
                else alert("No se consiguieron registros de factor accidente para el mes seleccionado");
           });
    ...
    function calculos()
    {
        if(empleado!=undefined&&dbsca!=undefined&&dbsalud!=undefined&&dbuf!=undefined&&dbfamiliares!=undefined&&dbimpuestos!=undefined&&salario!=undefined)
        {
        $(".form-field-box").show();
        ...
        var dias_trabajados = parseFloat($("#field-dias_trabajados").val()); //Modificado de "parseInt" a "parseFloat" por Victor Alarcon el 29/09/2014 ya que el calculo no consideraba dias trabajados con decimales. 
        var dias_mes = parseInt($("#field-dias_mes").val());
        var sueldo_base = parseInt($("#field-sueldo_base").val());
        //Calcular sueldo proporcional
        var sueldo_proporcional = dias_trabajados/dias_mes*sueldo_base;
        ...
        else if(total_imponible<dbfamiliares['tramo_bc'])
	...
	/* Modificado por Victor Alarcon el 01-10-2014 para resolver problema con total no imponible con decimales que no dejaba guardar la liquidacion en la DB */
	/*------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        var totalnoimponible = parseInt(locomocion) + parseInt(colacion) + parseInt(otras_asignaciones) + parseInt(familiares) + parseInt($("#field-indem_sustit").val()) + parseInt($("#field-indem_anos").val());
	...
	//Calcular fondo de pensiones
        ...
        //Calcular % de salud
		var salud = total_imponible <= tope_imponible?total_imponible*(parseFloat(empleado['por_salud'])/100):tope_imponible*(parseFloat(empleado['por_salud'])/100);
        ...
        var total_leyes_sociales = fondopensiones+salud+apv+inp+segurocesantia+adicional_salud;
	...
	total_descuentos = !isNaN(total_descuentos)?total_descuentos.toFixed(0):0;
        ...
        var ips = cot_accidente_trabajo-familiares;
	 ...
	 //Total fonasa
        var total_fonasa = ips<0?adicional_salud+salud+ips:adicional_salud+salud;     // Modificado el 30-09-2014 para resolver problema Asignacion familiar mayor que cotizacion SCA
        ...
        var total_leyes_sociales = total_fonasa<0?afp+afc:afp+afc+ips+total_fonasa;   // Modificado el 30-09-2014 para resolver problema total Fonasa en negativo (asignacion familiar mayor que cotizacion de salud y SCA)
        $("#field-total_leyes_sociales").val(!isNaN(total_leyes_sociales)?total_leyes_sociales.toFixed(0):0);
        //Costo total empleador
        var costo_total_empleador = total_leyes_sociales+otros_descuentos;
