...
//           var dias = new Date(parseInt(f[2]) || new Date().getFullYear(),parseInt(f[1]), 0).getDate();
	...
	//Calcular fondo de pensiones
        ...
        //Calcular % de salud
		var por_salud = empleado['id_caja']!==undefined&&empleado['sistema_salud_afiliado']==5?6.4:empleado['por_salud'];
        ...
        var ips = empleado['id_caja']!==undefined?cot_accidente_trabajo:cot_accidente_trabajo-familiares;
	 ...
	 //Total fonasa
		...
		var caja_comp = empleado['sistema_salud_afiliado']==5?(total_imponible*(parseFloat(empleado['por_caja'])/100))-familiares:0; //Esto es porque solo los adheridos a Fonasa cotizan en la caja;
		...
		if(total_fonasa>=0&&ips>=0&&total_caja>=0)
			total_leyes_sociales = afp+afc+ips+total_fonasa+total_caja;
