...
class Panel extends Main {
	...
	{             
            ...
            $crud->set_rules('rut','rut','required|regex_match[/^([0-9]+)-[k,K,1-9]$/]');
	...
	{            
            ...
            //Validations                
            $crud->required_fields('nombre','apellido_paterno','apellido_materno','empresa','domicilio','rut',
            'fecha_nacimiento','estado_civil','cargo','sexo','sueldo_base',
            'gratificacion','fecha_ingreso','cargas_familiares',
            'afp_afiliado','sistema_salud_afiliado','tipo_contrato',
            'dependiente','pais','ciudad','comuna','codigo_postal','nacionalidad','hora_entrada','hora_salida','cargo_descripcion',
            'dias_semana','contrato'); 
            ...
            $crud->callback_field('admin',function($val){return form_radio('admin',1,$val==1?TRUE:FALSE).' SI '.form_radio('admin',0,$val==0?TRUE:FALSE).' NO';});
            $crud->callback_column('admin',function($val){return $val==1?'SI':'NO';});
        ...
        public function clientes($x = '',$y = '')
	{
            ...
            'nombre','inicio_actividades','telefono',
            'celular','rut','calle','numero','dpto',
            'comuna','ciudad','region','pais','actividad_negocio',
            'actividad_economica','email_empresa','email_persona',
             	
'certificado_electronico','tipo_contribuyente','total_acciones',
            'cliente_activo','banco','cuenta','domicilio','estados_clientes');
        ...
        public function feriados($x = '',$y = '')
	{
            ...
            $crud->display_as('dias_anterior','Saldo Feriado anterior');   
        ...
        public function liquidaciones($x = '',$y = '')
	{
            ...
            $crud->fields(
                    'fecha_emision','empresa','empleado','fecha','dias_mes','dias_trabajados','sueldo_base','sueldo_base_proporcional','cantidad_horas','factor_hora_extra','hrs_extras','bonos_ficha','comisiones','bonos','aguinaldos','gratificaciones','preimponible','sub_totales_haberes','tope_imponible','total_imponible',
                    'locomocion','colacion','familiar','otras_asignaciones','indem_sustit','indem_anos','total_no_imponible',
                    'fondo_pensiones','cotiz_salud','ahorro_prev_voluntario','inp_mutual','cotiz_seg_ces','adicional_salud','total_leyes_soc','renta_afecta',
                    'imp_2cat','cta_ahorro_afp','anticipos','prestamos_ccaf','descuentos_prestamos','otros_descuentos','total_otros_descuentos',
                    'total_haberes','total_descuentos','alcance_liquido',
                    'total_leyes_sociales','total_impuestos','afp','cotizacion_obligatoria','seguro_inv','cuenta_ahorro','indemnizacion','afc','seguro_cesantia_trabajador','seguro_cesantia_empleador','mutualidad','cot_accidente_trabajo','total_ips',
                    'cotizacion_obligatoria_salud','cotizacion_adicional','total_fonasa','impuesto_trabajador','costo_total_empleador');
            //Fields                       
            //unsets            
            //Displays
            $crud->display_as('fecha','Mes/Año')
                 ...
                 ->display_as('inp_mutual','INP/MUTUAL')
                 ...
                 ->display_as('indemnizacion','Aporte Indemnizacion 4.11%')
            ...
            $crud->required_fields('empresa','fecha_emision','bonos_ficha','factor_hora_extra','empleado','fecha','dias_trabajados','cantidad_horas');            
            $this->db->where('reportes.tipo','1');
            $crud = $this->get_reportes($crud);
            return $crud;           
	}
        ...
        public function paginas($x = '',$y = '')
	{
            ...
            $crud->callback_column('visible',function($val){return $val==0?'NO':'SI';});
            $crud->callback_field('icono',function($val){return form_input('icono',$val,'id="field-icono"').' <a href="http://fortawesome.github.io/Font-Awesome/" target="_new">Iconos</a>';});
        ...
        public function banner($x = '',$y = '')
	{
        ...
        function importar_sueldos()
        {
            ...
            $crud->callback_column('archivo',function($val,$row){return '<a target="_new" href="'.base_url(get_instance()->router->fetch_class().'/procesar/'.$val.'/'.$row->empresa).'">'.$val.'</a>';});
            //Fields types            
            //Validations                
            $crud->unset_fields('id')
                 ->unset_columns('id');
        ...
        function importar_liquidaciones()
        {
            ...
            //Displays          
            $crud->set_field_upload('archivo','files');
            $crud->callback_column('archivo',function($val,$row){return '<a target="_new" href="'.base_url(get_instance()->router->fetch_class().'/procesar_liquidacion/'.$val.'/'.$row->empresa).'">'.$val.'</a>';});
            //Fields types            
            //Validations                
            $crud->unset_fields('id')
                 ->unset_columns('id');
        ...
        function isapre()
        {
            ...
            $crud->callback_field('plan_requerido',function($val){return form_dropdown('plan_requerido',array(1=>'SI',0=>'NO'),$val,'id="field-plan_requerido"');});
            $crud->callback_column('plan_requerido',function($val){return $val==0?'NO':'SI';});
            $crud->unset_columns('id')
                 ->unset_fields('id');            
            //Fields                        
            //unsets            
        ...
        function finiquitos_causales()
        {
            ...
            $crud->unset_fields('id')
                 ->unset_columns('id');
            $crud->callback_after_insert(function($post,$primary){get_instance()->db->update('trabajadores',array('fecha_egreso'=>  date("Y-m-d",strtotime(str_replace("/","-",$post['fecha'])))),array('id'=>$post['empleado']));});                       
            $this->db->where('reportes.tipo','1');
            $crud = $this->get_reportes($crud);
            return $crud;     
        }
        ...
        function notificaciones($x='',$y='')
        {
            ...
            if($x!='edit')$crud->field_type('siguiente_notificacion','invisible');
            $crud->unset_fields('id')
                 ->unset_columns('id');
            ...
            $crud->callback_before_insert(function($post){$post['siguiente_notificacion'] = $post['fecha'];});            
            $this->db->where('reportes.tipo','1');
            $crud = $this->get_reportes($crud);
            return $crud;   
        }
        ...
        function transacciones($x,$y)
        {
            ...
            $crud->unset_edit()
                 ->unset_delete();
            $crud->unset_columns('id');
            $crud->callback_column('operacion',function($val){return $val=='+'?'<span style="color:blue">'.$val.'</span>':'<span style="color:red">'.$val.'</span>';});
            $this->db->where('reportes.tipo','1');
            $crud = $this->get_reportes($crud);
            return $crud;
        }
        ...
        function imprimir_feriado($id)
        {
            ...
            {
                $this->db->select(
                'feriado.*,trabajadores.nombre as empnombre, trabajadores.apellido_paterno as empapellido, trabajadores.rut as emprif,
                 dbclientes112010.nickname as empresanombre, dbclientes112010.rut as empresarut');                
        ...
        function imprimir_finiquito($id)
        {
            ...
            {                
                $this->db->select('
                finiquitos.fecha, 
                dbclientes112010.nickname as empresa_nombre, dbclientes112010.rut as empresa_rut,
                ...
                dbclientes112010.calle as empresa_direccion, comunas.nombre as empresa_comuna,ci.nombre as ciudad_empresa,
                ...
                $this->db->join('ciudades ci','ci.id = dbclientes112010.ciudad','left');                
