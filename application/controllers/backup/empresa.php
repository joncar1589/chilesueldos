        ...
        public function index($url = 'main',$page = 0)
	{
        ...
        function get_salario()
        {
            ...
            if($this->form_validation->run())
            {
                ...
                $emp = $this->db->get_where('trabajadores',array('trabajadores.id'=>$this->input->post('id')));
        ...
        function get_liquidaciones()
        {
            ...
            if($this->form_validation->run())
            {
                ...
                $emp = $this->db->get_where('liquidaciones',array('liquidaciones.empleado'=>$this->input->post('id')));
                $emp->result();
        ...
        function get_feriados()
        {
            ...
            if($this->form_validation->run())
            {
                ...
                $this->db->order_by('id','DESC');
                $emp = $this->db->get_where('feriado',array('feriado.empleado'=>$this->input->post('id')));
        ...
        function getuf()
        {
            ...
            if($this->form_validation->run())
            {                
                $emp = $this->db->get_where('uf',array('iduf'=>$this->input->post('id')));
        ...
        function getfamiliares()
        {
            ...
            if($this->form_validation->run())
            {                   
                $f = $this->db->get_where('asignacionesfamiliares',array('ano'=>$this->input->post('ano')));
        ...
        function getimpuestos()
        {
            ...
            if($this->form_validation->run())
            {                   
                $this->db->where('EXTRACT(Month from fecha) = ',$this->input->post('mes'));
                $this->db->where('EXTRACT(Year from fecha) = ',$this->input->post('ano'));
                $f = $this->db->get('impuestos_trabajadores');
                echo $f->num_rows==0?json_encode(array('id'=>0)):json_encode($f->row());
            }            
        }
        ...
        function get_salario_minimo()
        {
            ...
            if($this->form_validation->run())
            {                   
                $this->db->where('fecha <= ',$this->input->post('ano').'-'.$this->input->post('mes').'-01');                
