...
class Main extends CI_Controller {
    ...
    function notificar($id = '', $reenviar='') {
        if (!empty($id)) {
            $this->db->where('id', $id);
        }
        $this->db->where('status', 1);

        //if (empty($reenviar)) { //Esto es para definir cuando se llama la notificacion desde el boton reenviar o desde el CRON 
                    ...
                    case 'Otros':
                        ...
                        foreach ($this->db->get('user')->result() as $u) {
                            ...
                            if (empty($des) || !strstr($u->email, $des))
                                $des = $u->email . ',';
                            echo $n->remitente . '<br/>' . $n->destinatarios . $des . '<div>' . utf8_decode($texto) . '</div>';
                            correo($des, $n->titulo . ' ' . $u->nombre, utf8_decode($texto), $n->remitente);
                            ...
                            //correo('joncar.c@gmail.com',$n->titulo.' '.$u->nombre,utf8_decode($texto),$n->remitente);
                        }
                        break;
                }
...
//                $texto = $this->getcontentmail($n, $user);
...
//    function reenviar_not($id = '', $reenviar='') {
//        
//        
//        
//    }
